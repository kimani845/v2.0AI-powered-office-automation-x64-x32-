...
# Assuming log_message and WPSAddin are defined elsewhere
def log_message(message, level="info"):
    print(f"[{level.upper()}] {message}")

# --- Main Registration/Unregistration Block (Improved with InprocServer32 and Unregister) ---
if __name__ == '__main__':
    log_message("=== Script Execution Started ===")
    log_message(f"Args: {sys.argv}")

    if len(sys.argv) > 1 and sys.argv[1].lower() == '/unregserver':
        # Step 1: Handle standard pywin32 unregistration
        log_message("Performing unregistration...")
        try:
            import win32com.server.register
            win32com.server.register.UnregisterServer(
                WPSAddin._reg_clsid_,
                WPSAddin._reg_progid_,
                WPSAddin._reg_progid_ + ".1" if hasattr(WPSAddin, '_reg_progid_') else None
            )
            log_message("✓ Standard COM unregistration completed.")
        except Exception as e:
            log_message(f"✗ FAILED to unregister COM server: {e}", "critical")

        # Step 2: Clean up WPS-specific add-in entry
        log_message("Removing WPS-specific add-in entry...")
        try:
            wps_path = f"Software\\Kingsoft\\Office\\Addins\\{WPSAddin._reg_progid_}"
            winreg.DeleteKeyEx(winreg.HKEY_CURRENT_USER, wps_path, 0, 0)
            log_message("✓ Removed WPS add-in entry.")
        except FileNotFoundError:
            log_message("ℹ WPS add-in entry not found.")
        except Exception as e:
            log_message(f"✗ FAILED to remove WPS add-in entry: {e}", "critical")

        # Step 3: Clean up CLSID entries for both registry views
        clsid_key_path = f"CLSID\\{WPSAddin._reg_clsid_}"
        reg_views = [winreg.KEY_WOW64_64KEY, winreg.KEY_WOW64_32KEY]
        for view in reg_views:
            try:
                winreg.DeleteKeyEx(winreg.HKEY_CLASSES_ROOT, clsid_key_path, view, 0)
                log_message(f"✓ Removed CLSID key for view {view}.")
            except FileNotFoundError:
                log_message(f"ℹ CLSID key not found for view {view}.")
            except Exception as e:
                log_message(f"✗ FAILED to remove CLSID key ({view}): {e}", "critical")

        print("SUCCESS: Add-in unregistered successfully. Please restart WPS Office.")
    else:
        # Step 1: Let pywin32 handle the standard registration
        log_message("Performing registration...")
        import win32com.server.register
        win32com.server.register.UseCommandLine(WPSAddin)

        # Step 2: If registering, clean up LocalServer32 and enforce InprocServer32
        if len(sys.argv) > 1 and sys.argv[1].lower() == '/regserver':
            log_message("Running post-registration cleanup...")
            clsid_key_path = f"CLSID\\{WPSAddin._reg_clsid_}"
            is_64bit_python = sys.maxsize > 2**32

            # Always handle both views so CLSID is available to both 32-bit and 64-bit apps
            reg_views = [winreg.KEY_WOW64_64KEY, winreg.KEY_WOW64_32KEY]

            # Path to pythoncom DLL (the COM entry point)
            pythoncom_dll = os.path.join(
                sys.base_prefix, "Lib", "site-packages", "pywin32_system32", f"pythoncom{sys.version_info.major}{sys.version_info.minor}.dll"
            )

            for view in reg_views:
                try:
                    # Remove LocalServer32 if present
                    localserver_path = os.path.join(clsid_key_path, "LocalServer32")
                    winreg.DeleteKeyEx(winreg.HKEY_CLASSES_ROOT, localserver_path, view, 0)
                    log_message(f"✓ Removed LocalServer32 key for view {view}.")
                except FileNotFoundError:
                    log_message(f"ℹ LocalServer32 not found for view {view}.")
                except Exception as e:
                    log_message(f"✗ Could not clean up LocalServer32 ({view}): {e}")

                # Force InprocServer32 with pythoncom DLL
                try:
                    inproc_path = os.path.join(clsid_key_path, "InprocServer32")
                    with winreg.CreateKeyEx(winreg.HKEY_CLASSES_ROOT, inproc_path, 0, winreg.KEY_WRITE | view) as key:
                        winreg.SetValueEx(key, None, 0, winreg.REG_SZ, pythoncom_dll)
                        winreg.SetValueEx(key, "ThreadingModel", 0, winreg.REG_SZ, "Both")
                    log_message(f"✓ Set InprocServer32 -> {pythoncom_dll} (view {view}).") 
                except Exception as e:
                    log_message(f"✗ FAILED to set InprocServer32 ({view}): {e}")

            # Step 3: Create the WPS-specific add-in entry
            log_message("Creating WPS-specific add-in entry...")
            try:
                path = f"Software\\Kingsoft\\Office\\Addins\\{WPSAddin._reg_progid_}"
                with winreg.CreateKeyEx(winreg.HKEY_CURRENT_USER, path, 0, winreg.KEY_WRITE) as key:
                    winreg.SetValueEx(key, "Description", 0, winreg.REG_SZ, WPSAddin._reg_desc_)
                    winreg.SetValueEx(key, "FriendlyName", 0, winreg.REG_SZ, "AI Assistant")
                    winreg.SetValueEx(key, "LoadBehavior", 0, winreg.REG_DWORD, 3)
                    winreg.SetValueEx(key, "CLSID", 0, winreg.REG_SZ, WPSAddin._reg_clsid_)
                log_message("✓ SUCCESS: WPS add-in entry created.")
                print("SUCCESS: Add-in registered correctly. Please restart WPS Office.")
            except Exception as e:
                log_message(f"✗ FAILED to create WPS add-in entry: {e}")
                print(f"FAILED to create WPS add-in entry: {e}")

            # Step 4: Verify registry values
            log_message("Verifying registry entries...")
            try:
                progid_path = f"Software\\Kingsoft\\Office\\Addins\\{WPSAddin._reg_progid_}"
                with winreg.OpenKey(winreg.HKEY_CURRENT_USER, progid_path) as key:
                    clsid, _ = winreg.QueryValueEx(key, "CLSID")
                    if clsid == WPSAddin._reg_clsid_:
                        log_message(f"✓ Verified CLSID mapping: {clsid}")
                    else:
                        log_message(f"✗ MISMATCH: Registry CLSID = {clsid}, expected {WPSAddin._reg_clsid_}", "critical")
            except Exception as e:
                log_message(f"✗ Verification failed: {e}", "critical")
